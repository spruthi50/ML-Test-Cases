...
def Trainfunc():
 ...
 ridge=Ridge()
 global ridgeModel
 ridgeModel=GridSearchCV(ridge,param_grid,cv=10)
 ridgeModel.fit(XTrain,yTrain)
 
 predicted=ridgeModel.predict(XTest)
 ridgeModelMse=mean_squared_error(yTest,predicted)
 output_list.append("MSE::")
 output_list.append(math.sqrt(ridgeModelMse))
 output_list.append("R2 Score")
 output_list.append(r2_score(yTest,predicted))
 output_list.append("Best Parameter")
 output_list.append(ridgeModel.best_params_)
...
def Testfunc():
 ...
 Trainfunc()
 predicted=ridgeModel.predict(xValues)
